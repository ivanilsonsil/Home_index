...
# criando função para perfil do usuario
@login_required(login_url='/auth/logar')
def perfil(request):
    if request.method == "GET":
        return render(request, 'perfil.html')

        # estamos buscando do banco de dados todos os jobs
        # com filtro
...
def enviar_projeto(request):
    arquivo = request.FILES.get('file')#receber arquivo
    id_job = request.POST.get('id') #pegar dados do form por meio POST


    job = Jobs.objects.get(id=id_job) # Pegar dados do banco jobs
